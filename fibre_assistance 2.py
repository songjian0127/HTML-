...
def _vmr_html_to_csv_like_tempfile(html_text):
    ...
    def clean_txt(tag):
        if not tag: return ""
        # Replace line breaks with spaces for single-line fields, or keep specific formatting if needed
        return " ".join(tag.get_text(" ", strip=True).split())

    # --- 1. Fibre Trace Summary ---
    # CSV Structure:
    # Row 6: "Fibre Trace Summary"
    # Row 7: Headers (A End, Name, Z End)
    # Row 8+: Data
    ...
    # CSV Structure:
    # "Calculated Loss"
    # Headers (#Splices, #Patches, Total Length(m), Est. db Loss...)
    # Data
    writer.writerow(["Calculated Loss"])
    tbl_loss = soup.find("table", id="gvDbLoss")
    if tbl_loss:
        # The loss table is often nested. Look for the inner data rows.
        # Based on structure: The id is on the outer table, inner table contains data.
        # We'll just grab all text recursively for simplicity as it's a single row.
        rows = tbl_loss.find_all("tr")
        # Usually the first nested table row has headers, second has data.
        # We will try to extract structured data if possible.
        extracted_rows = []
        for tr in rows:
            cells = [clean_txt(td) for td in tr.find_all(["th", "td"])]
            if cells and any(cells): # Skip empty
                extracted_rows.append(cells)
        
        # Deduplicate rows that might appear due to nesting (naive approach)
        seen = []
    ...
    # CSV Structure:
    # "Fibre Trace Details"
    # Header: (misaligned in CSV, but we will write a standard header row)
    # Data Block per item:
    #   Row A: ID | A End | Name | Z End | ... | C/D | EO | Length
    #   Row B: (Spacer/Extra Info)
    #   Row C: (Fibre count info)
    
    writer.writerow(["Fibre Trace Details"])
    
    # Write a header row that aligns with the column indices used in logic
    ...
    if tbl_details:
        ...
        for tr in trs[start_idx:]:
            ...
            # Note: The HTML usually has hidden columns. We must respect the visual index.
            # Visual Index 1 -> ID
            # Visual Index 2 -> A End
            # Visual Index 3 -> Name (Cable)
            # Visual Index 4 -> Z End
            # Visual Index 8 -> C/D (The nested table)
            # Visual Index 9 -> EO
            # Visual Index 10 -> Length
            
            # Helper to get text safely by index
            def get_td(idx): return tds[idx] if idx < len(tds) else None
            
            # 1. ID
            item_id = clean_txt(get_td(1))
            
            # 2. A End (First line only usually required for summary)
            a_end_full = clean_txt(get_td(2))
            
            # 3. Name (This is complex: "CableName(#XX) ... XXm, YYfibres ...")
            name_td = get_td(3)
            name_full = clean_txt(name_td)
            
            # Extract specific cable info for the auxiliary rows
            # Find Length and Fibres in the text "636.00m, 36fibres"
            m_len = re.search(r"(\d+(?:\.\d+)?)\s*m", name_full, re.IGNORECASE)
            m_fib = re.search(r"(\d+)\s*fibres", name_full, re.IGNORECASE)
            
            len_str = m_len.group(1) if m_len else ""
            fib_str = m_fib.group(1) if m_fib else ""
            
            # Extract clean cable name (first line)
            # If there's an anchor tag, use that text, otherwise split by newline
            cable_name = ""
            if name_td:
                ...
                if anchor:
                    ...
                    cable_name = name_full.split(" ")[0] # Fallback
                
                # Append (#xx) if present in full text but not in anchor
            ...
            # 8. C/D (Connect/Disconnect) - Needs parsing the nested table images
            cd_td = get_td(8)
            cd_text = ""
            if cd_td:
                # The C/D info is in a nested table with images
                sub_rows = cd_td.find_all("tr")
                cd_parts = []
                for sr in sub_rows:
                    img = sr.find("img")
            ...
            row3 = [""] * 12
            if len_str and fib_str:
                row3[3] = f"{len_str}m, {fib_str}fibres" # Place in Name column
            writer.writerow(row3)
            
            # Row 4: Spacer or Work Info (Optional, keeping simple for now)
            writer.writerow([])
...
# --- NEW: add inside class FibreProcessor (e.g., after process_csv) ---
    ...
    def process_data(self):
        ...
        try:
            ...
            # --- Check Database Availability BEFORE Loop ---
            db_available = False
            if os.path.exists(self.db_path):
                try:
                    ...
                    if cur_check.fetchone()[0] > 0:
                        ...
                        self.log("ERROR: Database exists but table 'Cable' is missing.")
                    conn_check.close()
                except Exception as e:
                    self.log(f"ERROR: Database check failed: {e}")
            else:
                self.log("WARNING: database.db not found. Run 'Fibre Database Update' tab.")
            ...
            for i in range(1, len(processed_data)):
                ...
                # Only try to fetch Segment ID if DB is available
                ...
                if key in segid_cache:
                    ...
                    try:
                        ...
                        self.log(f"DB Error fetching cable {key}: {e}")
            ...
            if self.crawl_enabled.get():
                if not db_available:
                    self.log("Skipping VMR Cross-Section crawl because Database is missing (cannot map Cable Name -> Segment ID).")
                elif not to_crawl:
                    self.log("Connect VMR is ON, but no valid Segment IDs found to crawl.")
                else:
                    ...
                    for idx, (seg_id, url) in enumerate(to_crawl, start=1):
                        try:
                            self.log(f"Crawling {idx}/{len(to_crawl)}: {seg_id}")
                            ...
                            if resp.status_code == 200:
                                ...
                                self.log(f"Failed {seg_id}: HTTP {resp.status_code}")
                        except Exception as e:
                            self.log(f"Error crawling {seg_id}: {e}")
                        finally:
            ...
            try: ui_comm = cols.index("Commentary")
            except: ui_comm = len(cols)-1
            ...
            for i in range(1, len(processed_data)):
                ...
                # --- 2. Database Checks (Only if DB available) ---
                if cursor:
                ...
                # --- 3. Splice Checks (Only if DB available) ---
